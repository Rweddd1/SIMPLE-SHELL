...
Local IP:   {ip}
            ...
            print(f"ERROR: {e}")
#--------------- FIX PA
    def available_wifi_windows():
        try:
            check_output = subprocess.check_output(["netsh", "wlan", "show", "network"]).decode("ascii")
            ssids_w = []
            for i in check_output.split('\n'):
                if "SSID" in i and "BSSID" not in i:
                    ssids_w = i.split(':')[1].strip()
                    if ssids_w:
                        ssids_w.append(ssids_w)
            return ssids_w
        except subprocess.CalledProcessError as e:
            print(f'ERROR {e}')
            return []
            
    def available_wifi_nonwindow(interface="wlan 0"):
        try:
            command_output = subprocess.check_output(["sudo", "iwlist", interface, "scanning"]).decode("utf-8")
        
            ssids_nw = re.findall(r'ESSID:"(.+?)"', command_output)
            return ssids_nw
        except subprocess.CalledProcessError as e:
            print(f"Error executing command: {e}")
            print("Ensure you have 'iwlist' installed and run with appropriate permissions (e.g., using sudo).")
            return []
        
    def ssids(self):
        sytstem_os = platform.system().lower()
        if "windows" in sytstem_os:
            return Network.available_wifi_windows()
        else:
            return Network.available_wifi_nonwindow()